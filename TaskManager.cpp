...
void TaskManager::init() {
    ...
    while (Application::state != Application::State::STOPPING)
    {

        {
            ...
            MarketFilter filter(m_marketIds);
...

        {
            ...
            std::forward_list<std::string> marketIds;
            for (auto m :  m_marketIds) {
                marketIds.push_front(m);
            }
        ...
        {
            CurrentOrderSummaryReport report;
            ...
            for (const CurrentOrderSummary& summary : report.currentOrders) {
                std::cout << summary.betId << std::endl;
                BetfairOrder *obj = bd.orderModel().getById(summary.betId);
                if (obj == nullptr) {
                    BetfairMarket* market = bd.marketModel().getById(summary.marketId);
                    BetfairRunner* runner = bd.runnerModel().getById(std::to_string(summary.selectionId));
                    if (market == nullptr)
